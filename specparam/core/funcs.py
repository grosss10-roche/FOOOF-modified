...
def lorentzian_with_expo_function(xs, *params):
    """Exponential fitting function, for fitting aperiodic component with a 'knee' and an exponential function.
    ...
    xs : 1d array
        Input x-axis values.
    *params : float
        Parameters (offset, knee, exp, slope) that define Lorentzian function with an exponential function:
    ...
    offset, knee, exp, slope = params
    ys = offset - slope*xs - np.log10(knee + xs**exp)
...
def get_ap_func(aperiodic_mode):
    ...
    elif aperiodic_mode == 'knee_expo':
        ap_func = lorentzian_with_expo_function
    else:
